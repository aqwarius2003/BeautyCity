...
def start(update: Update, context: CallbackContext):
    ...
    if admin.is_admin(user_id):
        admin.show_admin_menu(update, context)
    else:
        update.message.reply_text("Пользователь не администратор.")
...
def show_specialists(update: Update, context: CallbackContext):
    with open('menu.json', 'r', encoding='utf-8') as file:
        menu_data = json.load(file)

    specialists = [item for item in menu_data if item['callback_data'] == 'specialists']
    message_text = '\n'.join([f"{specialist['name']}: {specialist.get('description', 'Описание недоступно')}" for specialist in specialists[0]['submenu']])
    update.message.reply_text(message_text)
    show_big_keyboard(update, context, update.message.chat_id)
...
def main():
    ...
    dp.add_handler(MessageHandler(Filters.text("Мастера"), show_specialists))
    dp.add_handler(MessageHandler(Filters.text("Администратор"), show_admin_message))
    dp.add_handler(MessageHandler(Filters.text & ~Filters.command, admin.handle_admin_choice))
