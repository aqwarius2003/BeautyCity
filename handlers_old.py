...
from messages import super_start, terms_read_message, welcome_back_message, admin_message, salons_info
import messages
from keyboard import button_main_menu, button_salons
...
def show_salons_menu(update: Update, context: CallbackContext) -> None:
    keyboard = build_salons_menu_keyboard()
    update.callback_query.message.reply_text(
        text="Выберите салон:",
        reply_markup=keyboard
    )

# Функция для построения клавиатуры меню салонов
def build_salons_menu_keyboard():
    keyboard = []
    for key, label in button_salons.items():
...
# Обработчик для кнопок салонов
def salon_button_handler(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    query.answer()

    salon_id = query.data  # Это будет 'salon1', 'salon2', или 'salon3'
    print(f'Выбран {salon_id}')
    salon_info = messages.salons_info.get(salon_id, "Информация о салоне не найдена")
    keyboard = build_salon_action_keyboard(salon_id)
...
# Функция для построения клавиатуры действий с салоном
def build_salon_action_keyboard(salon_id):
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("Записаться сюда", callback_data=f"book_{salon_id}")],
        [InlineKeyboardButton("Назад", callback_data="back_to_salons_menu")]
    ])
    return keyboard

# Обработчик для кнопки "Записаться сюда"
def book_salon_handler(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    query.answer()

    # Split the query.data and handle the case where it might not split correctly
    data_parts = query.data.split('_')
    if len(data_parts) < 2:
        # Handle the case where query.data doesn't split as expected
        context.bot.send_message(
            chat_id=query.message.chat_id,
            text="Ошибка: Не удалось обработать запрос на запись в салон."
        )
        return

    salon_id = data_parts[1]  # Get the salon_id ('salon1', 'salon2', 'salon3')

    # Here you can continue with your logic for booking the salon
    context.bot.send_message(
        chat_id=query.message.chat_id,
        text=f"Вы успешно записаны в салон {button_salons.get(salon_id, 'неизвестный салон')}"
    )

    # Show the salons menu again or perform other actions
    show_salons_menu(update, context)
# Обработчик для кнопки "Назад" из меню салонов
def back_to_salons_menu_handler(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    query.answer()
    # Показываем снова меню салонов
    show_salons_menu(update, context)
